...
{{< downloadthis index.R dname="week_script" label= "Download Week 3 R script" icon="database-fill-down" type="success" >}}
...
The correlation coefficient can be computed in R by `summarising` our data using the `cor` function as follows:
...
Lastly, besides our usual diagnostic plots to checks we need to be carefull with the collinearity among our predictors since a high collinearity value may inflate the parameter uncertainty.
