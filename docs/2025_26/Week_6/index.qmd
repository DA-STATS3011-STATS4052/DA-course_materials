...
|:--------------:|:--------------:|:----------------------:|:--------------:|
|  Normal   | $y_i\overset{\text{indep}}\sim \text{N}(\mu_i,\sigma^2),$ | $\boldsymbol{x}_i^\top\boldsymbol{\beta} =\beta_0 + \beta_1x_i + \beta_2x_i + \ldots$ |                  $g(\mu_i)=\mu_i$                  |
| Logistic  |    $y_i\overset{\text{indep}}\sim \text{Bin}(1,p_i),$     | $\boldsymbol{x}_i^\top\boldsymbol{\beta} =\beta_0+ \beta_1x_i + \beta_2x_i + \ldots$  | $g(\mu_i) = \log \left( \frac{p_i}{1-p_i} \right)$ |
...
and can be fitted in`R` as follows:
...
mod1.odds <- model %>% broom::tidy(exponentiate = T,conf.int=T)
...
-   For `age` we have an odds of `r round(mod1.odds$estimate[2], 2)`, which indicates that for every 1 unit increase in age, the odds of the teaching instructor being male increase by a factor of `r round(mod1.odds$estimate[2],2)` (equivalently to say that the odds of being a male are 6% greater than a female for every unit increase in age).
...
&= \exp\left(`r round(coef(model)[1],2)` + `r round(coef(model)[2],2)` \cdot 45\right) = `r round(exp(coef(model)[1] + coef(model)[2] * 45), 2)`. \nonumber \end{align}
```
...
Which is exactly the estimate we get in @tbl-summaries_Odds_conti below.
...
```{=tex}
\begin{align}
\ln\left(\frac{\hat{p_i}}{1-\hat{p_i}}\right) &= `r mod2coefs[1]` + `r mod2coefs[2]` \cdot \mathbb{I}_{\text{ethnicity}}(\text{not minority}), \nonumber
\end{align}
```
...
2.  Recall that the **log-odds** of an instructor **not** belonging to the minority group (i.e. `not minority`) being a male are $\alpha +\beta_{\text{ethnicity}}$. On the odds scale this is:
...
3.  Now, the odds of an instructor that is **no**t in the minority group being male against the odds of an instructor that comes from the `minority` group is given by the ratio of the odds we just calculated:
    ...
    &= \exp\left(\beta_{\text{ethnicity}}\right) = \exp\left(`r round(mod2coefs[2], 2)` \right) \\
    &= `r round(exp(mod2coefs[2]), 2)`. \nonumber 
    \end{align}
    ```

Which is the coefficient estimate we got from the model summaries ( @tbl-summaries_Odds_categorical ). This means that instructors that are not in the minority groups are significantly 1.93 times more likely to be males compared to instructors in the minority group.
...
Great work today! You’ve reached the end this session - why not take your skills even further? Level up by tackling the [extra challenge tasks](about.qmd).
