...
Produce a table that computes the mean and the standard deviation of the petal length grouped by species. Round each value to 2 decimals and use appropriate column headings.

`r hide("Take hint")`

Use the `summarise` function to compute the mean and standard deviation of the `Petal.Length` variable grouped `.by` the `Sepcies` variable. Then use the `gt()` function to produce the table using `fmt_number` and `cols_label` to modify the number of decimals and column labels accordingly.
...
iris %>% 
  ...
  fmt_number(decimals = 2) %>%
  cols_label(
    mean.petal = html("Mean petal length (cm)"),
    sd.petal = html("Std. Dev. petal length (cm)")
    )
...
Tables can also be produced 'by hand' in Quarto. Here is an example of how we can recreatethe table above corresponding to the first 5 rows of the `iris` data.
...
``` latex
    $$\mathbb{I}_{\mathrm{Male}}(x)=\left\{
                \begin{array}{ll}
                  1 ~~~ \mathrm{if Gender of} ~ x \mathrm{th observation is Male},\\
                  0 ~~~ \mathrm{Otherwise}.\\
                \end{array}
              \right.$$
```
