...
You can find below an example of a report produced with Quarto relating to fitting a regression model using one numerical explanatory. You can toggle the `</> Code` button in the html file to see the source code that produced this file. It is advised to have this document open while working through the remaining sections in order to see examples put into practice.
...
The following sections will take you through the different steps required to produce this report in Quarto. For creating a new Quarto file from scratch see Section 1.1, otherwise move onto Section 2.
...
From here select **Document** and the **Default Output Format** as **HTML** (**PDF** output is also possible but you need the latest version of `MikTex` to be installed if you are a Windows user). Give your document a title and select OK. This will open within RStudio an empty `.qmd` file.
...
Create a new quarto document by clicking on the `New File` icon ![](images/new_doc.png){width="23"} , chage the title to *"Data Analysis: Example Report"* and add numbered-level 1 headings for each of the following project sections to the Quarto document you just created:
...
References can be included by clicking on `Insert▾➠ @Citation` then on the pop-up window you can look up for a reference, add the citation for an R package that is installed in your local computer or add a citation based on the DOI code of a paper or book:
...
Then, add appropriate labels for each section. Copy and paste the following text under the `Introduction` section:
...
Then, change the species names to *italics* format and add a citation `[CITE]` for the iris data set using the following doi: `10.24432/C56C76` . Lastly, change the text refering to to Section 2, 3 and 4 with clickable hyperlinks to each of these sections. Render ![](images/rstudio-render-button.png){width="21" height="17"} your Quarto document and check the HTML output.
...
Then, `@fig-scat` should be used in the text description to refer to this plot.
...
$$y_i = \alpha + \beta x_i + \epsilon_i, ~~~~ \epsilon_i \sim N(0, \sigma^2),$$ can be specified using the following:
...
We can use the `\begin{cases}` from the `amsmath` Latex package (available already in Quarto) to replicate this expression:
